...
#define pushinteger(n,u) lpb_pushinteger((L), (n), (u), LS->int64_mode)
    ...
    case PB_Tint64: case PB_Tuint64: case PB_Tsint64:
        if (pb_readvarint64(s, &v.u64) == 0)
            luaL_error(L, "invalid varint value at offset %d", (int)pb_pos(*s)+1);
        switch (type) {
    ...
    case PB_Tsfixed32:
        if (pb_readfixed32(s, &v.u32) == 0)
            luaL_error(L, "invalid fixed32 value at offset %d", (int)pb_pos(*s)+1);
        switch (type) {
    ...
    case PB_Tsfixed64:
        if (pb_readfixed64(s, &v.u64) == 0)
            luaL_error(L, "invalid fixed64 value at offset %d", (int)pb_pos(*s)+1);
        switch (type) {
...
static int lpb_unpackscalar(lua_State *L, int *pidx, int top, int fmt, pb_Slice *s) {
    ...
    case 'c':
        ...
        if (pb_readslice(s, (size_t)v.lint, v.s) == 0) return luaL_error(L,
                "invalid sub string at offset %d", (int)pb_pos(*s)+1);
        push_slice(L, *v.s);
        break;
    default:
        return 0;
...
static int Lslice_enter(lua_State *L) {
    ...
    if (lua_isnoneornil(L, 2)) {
        argcheck(L, pb_readbytes(&s->curr, &view) != 0,
            1, "bytes wireformat expected at offset %d", (int)pb_pos(s->curr)+1);
        view.start = view.p;
